...
from src.config import COMMON_HEADERS, URLS
    ...
    for account in accounts:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if roles_result["success"]:
                ...
                for role in roles:
                    print(
                        f"  - {role.get('game_biz')}: {role.get('nickname')} ({role.get('region')}) - UID: {role.get('game_uid')}"
                    )
            else:
            ...
            for game in Game:
                ...
                result = await safe_api_call(session, url, headers, params=params)

...
async def fetch_uid(session: aiohttp.ClientSession, account: Account, game: Game, region_code: str) -> str | None:
...
async def exchange_cdkey(
    session: aiohttp.ClientSession, account: Account, game: Game, region_code: str, uid: str, cdkey: str
) -> dict[str, Any]:
    """Nhập 1 redeem code
...
async def run_redeem_for_account(
    session: aiohttp.ClientSession, account: Account, cdkeys: dict[Game, list[str]], uids: dict[Game, dict[str, str]]
) -> dict:
    ...
    tasks = [
        _redeem_game_for_account(session, account, game, codes, game_uids) for game, codes, game_uids in games_to_redeem
    ]

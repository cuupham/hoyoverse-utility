...
class StarRailAPI(BaseAPI):
    
    _URL_PUBLIC         = 'https://sg-public-api.hoyolab.com/event/luna'
    URL_HKRPG           = f'{_URL_PUBLIC}/hkrpg/os'
    URL_EXCHANGE_CDKEY  = 'https://public-operation-hkrpg.hoyolab.com/common/apicdkey/api/webExchangeCdkeyHyl'

    ACT_ID      = BaseAPI.PARAMS['sr']['act_id']
    GAME_BIZ    = BaseAPI.PARAMS['sr']['game_biz']

    @classmethod
    def get_sign_info(cls, cookies:dict):
        return cls._get(
            f'{cls.URL_HKRPG}/info',
            cookies=cookies,
            headers={
    ...
    def exchange_cdkey(cls, cookies, region_choice, uid, cdkey):
        return cls._get(
            ...
            params={
                ...
                'lang': 'en',
                'region': cls.PARAMS['sr']['region'][region_choice],
                't': unix_ms(),
                'uid': uid
            }
        )